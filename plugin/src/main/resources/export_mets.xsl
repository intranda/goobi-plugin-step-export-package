		...
		<record>
			...
			<xsl:for-each select="/mets:mets/mets:dmdSec">
				<element>
					...
					<xsl:for-each select="mets:mdWrap/mets:xmlData/mods:mods/mods:titleInfo/mods:title">
						<title>
							<xsl:value-of select="."/>
						</title>
					</xsl:for-each>
					<xsl:if test="(./@ID='DMDLOG_0000')">
				        <publishing>
							<place>
								<xsl:value-of select="mets:mdWrap/mets:xmlData/mods:mods/mods:originInfo/mods:publisher"/>
							</place>
							<house>
								<xsl:value-of select="mets:mdWrap/mets:xmlData/mods:mods/mods:originInfo/mods:place/mods:placeTerm"/>
							</house>
							<year>
								<xsl:value-of select="mets:mdWrap/mets:xmlData/mods:mods/mods:originInfo/mods:dateIssued"/>
							</year>
						</publishing>
				    </xsl:if>
